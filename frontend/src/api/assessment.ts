...
const transition = (id: number, target_status: PlanStatus, version: number, reason: string, action: 'submit' | 'approve', confirmed_flags?: string[]) => request<DecompressionAssessment>(`/assessments/${id}/${action}`, { method: 'POST', body: JSON.stringify({ target_status, version, reason, confirmed_flags }) })
...
export const approveAssessment = (id: number, version: number, reason: string, confirmedFlags: string[]) => transition(id, 'approved_for_training', version, reason, 'approve', confirmedFlags)
