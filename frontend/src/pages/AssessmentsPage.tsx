...
export function AssessmentsPage() {
  ...
  const requiredItems = useMemo(() => selected?.risk_review.items.filter((item) => item.required) ?? [], [selected?.risk_review])
  const checkedFlags = requiredItems.filter((item) => checked[item.code]).map((item) => item.code)
  const allConfirmed = requiredItems.length > 0 && checkedFlags.length === requiredItems.length
  useEffect(() => { setChecked({}) }, [selected?.id])
  const transition = async (kind: 'submit' | 'approve') => {
    ...
    try {
      ...
      else await assessments.approve(selected.id, plan.version, reason, checkedFlags)
      ...
      <div className="assessment-layout">
        ...
        <section className="assessment-detail">
          {selected ? <>
            ...
            <div className="review-bar"><PlanStatusBadge status={selected.assessment_status} /><TextField label="Review reason" value={reason} onChange={(event) => setReason(event.target.value)} fullWidth />{isPlanner && selected.assessment_status === 'modeled' && <Button variant="contained" startIcon={<Send size={17} />} disabled={busy || reason.length < 3} onClick={() => void transition('submit')}>Submit</Button>}{isSupervisor && selected.assessment_status === 'pending_supervisor_review' && <Button variant="contained" color="secondary" startIcon={<CheckCheck size={17} />} disabled={busy || reason.length < 3 || !allConfirmed} title={allConfirmed ? 'Approve with the confirmed risk set' : 'Confirm every required risk flag below first'} onClick={() => void transition('approve')}>Approve training</Button>}</div>
            <section className="risk-section"><div className="subheading">Risk evidence <span>{selected.risk_flags.length}</span></div>
              <div className="review-summary"><span>Supervisor confirmations <b>{selected.risk_review.required_count - selected.risk_review.pending_count}/{selected.risk_review.required_count}</b></span>{selected.risk_review.pending_count > 0 && <span className="pending-pill">{selected.risk_review.pending_count} unconfirmed</span>}{selected.risk_review.confirmed_by && <span>Confirmed by {selected.risk_review.confirmed_by}{selected.risk_review.confirmed_at ? ` · ${new Date(selected.risk_review.confirmed_at).toLocaleString()}` : ''}</span>}</div>
              <div className="risk-list">{selected.risk_flags.map((flag) => { const item = selected.risk_review.items.find((entry) => entry.code === flag.code); const canConfirm = isSupervisor && selected.assessment_status === 'pending_supervisor_review' && !!item?.required && !item.confirmed; return <article className={`risk-row risk-${flag.band}`} key={flag.code}><ShieldAlert size={18} /><div><strong>{flag.code.replaceAll('_', ' ')}</strong><p>{flag.message}</p><small>{flag.evidence}</small>{item?.required && <small className="confirm-line">{item.confirmed ? `Confirmed by ${item.confirmed_by}${item.confirmed_at ? ` · ${new Date(item.confirmed_at).toLocaleString()}` : ''}` : 'Awaiting supervisor confirmation'}</small>}</div>{canConfirm ? <label className="confirm-check"><input type="checkbox" checked={!!checked[flag.code]} onChange={(event) => setChecked((current) => ({ ...current, [flag.code]: event.target.checked }))} />Confirm</label> : <span>{item?.confirmed ? '✓ ' : ''}{flag.band}</span>}</article> })}</div></section>
